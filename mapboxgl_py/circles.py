...
class CircleViz(object):
    ...
    def __init__(self, access_token, div_id='map', width='100%', height='500px'):
    ...
    def createViz(data, vizProperty, styleUrl="mapbox://styles/mapbox/light-v9",
        ...
        <!DOCTYPE html><html>
            ...
            accessToken=super(Circles,self).getToken(),
            styleUrl=self.styleUrl,
            center=self.center,
            zoom=self.zoom,
            geojson_data=json.dumps(self.data),
            vizProperty=self.vizProperty,
            colorStops=self.colorStops
        )

    def displayViz(self):
        # Load the HTML iframe
        map_html = super(Circles,self).repr_html(self.html_data)

        # Display the iframe in the current jupyter notebook view
        super(Circles,self).display(HTML(map_html))
